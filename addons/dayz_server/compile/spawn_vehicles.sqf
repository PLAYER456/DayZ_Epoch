...
while {count AllowedVehiclesList > 0} do {
	...
	diag_log("DEBUG: Spawning Vehicle: " + str(_vehicle));
...
if (count AllowedVehiclesList == 0) then {
	...
	if (_isShip or _isAir) then {
		...
		switch (true) do {
			case (_vehicle in ATVTypes): {_position = ATVLocations call BIS_fnc_selectRandom;};
			case (_vehicle in MotorbikeTypes): {_position = MotorbikeLocations call BIS_fnc_selectRandom;};
			case (_vehicle in HeliArmedTypes): {_position = HeliArmedLocations call BIS_fnc_selectRandom;};
			case (_vehicle in HeliUnarmedLocation): {_position = HeliUnarmedLocation call BIS_fnc_selectRandom;};
			case (_vehicle in HeliTransportTypes): {_position = HeliTransportLocations call BIS_fnc_selectRandom;};
			case (_vehicle in PlaneTypes): {_position = PlaneLocations call BIS_fnc_selectRandom;};
			case (_vehicle in BicyclesTypes): {_position = BicyclesLocations call BIS_fnc_selectRandom;};
			case (_vehicle in MilitaryGroundTypes): {_position = MilitaryGroundLocations call BIS_fnc_selectRandom;};
			case (_vehicle in CivilianGroundTypes): {_position = CivilianGroundLocations call BIS_fnc_selectRandom;};
			case (_vehicle in BusTypes): {_position = BusLocations call BIS_fnc_selectRandom;};
			case (_vehicle in TractorsTypes): {_position = TractorsLocations call BIS_fnc_selectRandom;};
			case (_vehicle in TrucksTypes): {_position = TrucksLocations call BIS_fnc_selectRandom;};
			case (_vehicle in BoatsTypes): {_position = BoatsLocations call BIS_fnc_selectRandom;};
		};
	...
	/*if ((count _position) == 2) then {
		_position set [2,0];*/
		_dir =  vehLocation select 0;        //round(random 180); 
